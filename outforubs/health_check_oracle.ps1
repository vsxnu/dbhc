...
foreach ($fqdn in $fqdnList) {
    ...
    if ($matchingEntries.Count -gt 0) {
        foreach ($entry in $matchingEntries) {
            ...
            try {
                $process = Start-Process -FilePath "sqlplus" -ArgumentList "$username/$password@$entry @$sqlFilePath" -PassThru -NoNewWindow -RedirectStandardOutput "NUL"
...
=============================================
Server: $fqdn
Status: No matching TNS entries found
=============================================
"@
    }
}
...
$output = @"
Date: $(Get-Date)
Oracle HC Report for multiple servers:
