...
$scriptBlock = {
    ...
    # RUN SQLCMD with increased column width and formatted output
    $sqlOutput = sqlcmd -S $fqdn -Q "EXEC DBServices..usp_healthchk" -W -h-1 -w 1000 -s"|" -y0 | ForEach-Object {
        $_ -replace '\|', ' | '
    } | Out-String
...
<body>
    ...
    <p>Date: $date</p>
    <p>Server: $fqdn</p>
    <pre>$($sqlOutput -replace '<', '&lt;' -replace '>', '&gt;')</pre>
</body>
</html>
"@
